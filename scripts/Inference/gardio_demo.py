...
DEFAULT_SYSTEM_PROMPT = """আপনি একজন সহায়ক সহকারী। You are a helpful assistant."""
        ...
        with gr.Column(scale=4):
            with gr.Column(scale=3):
                system_prompt_input = gr.Textbox(
                    show_label=True,
                    label="System prompt (modification is only valid before the conversation starts or after clearing the history, and is invalid during the conversation)",
                    placeholder=DEFAULT_SYSTEM_PROMPT,
                    lines=1).style(
                    container=True)
                negative_prompt_input = gr.Textbox(
                    show_label=True,
                    label="Reverse prompt (modification is only valid before the start of the conversation or after clearing the history, and is invalid during the conversation)",
            ...
            with gr.Column(scale=12):
                user_input = gr.Textbox(
                    ...
                    placeholder="Shift + Enter Send a message...",
                    lines=10).style(
                    container=True)
